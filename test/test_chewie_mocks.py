...
def _future(value):
    """Wrap object in future for async method mock."""
    fut = asyncio.Future()
...
class ChewieWithMocksTestCase(asynctest.TestCase):
    ...
    async def test_eap_packet_in_goes_to_new_state_machine(self, state_machine, ethernet_parse): #pylint: disable=invalid-name
        """test EAP packet creates a new state machine and is sent on"""
        self.chewie.eap_socket = Mock(**{'receive.return_value': _future('message from socket')})
        ethernet_parse.side_effect = return_if(
            ('message from socket',),
            (FakeEapMessage('fake src mac'), 'fake dst mac')
            )
        await self.chewie.receive_eap_messages()
    ...
    async def test_eap_output_packet_gets_packed_and_sent(self, ethernet_pack): #pylint: disable=invalid-name
        """test EAP packet creates a new state machine and is sent on"""
    ...
    async def test_radius_packet_in_goes_to_state_machine(self, state_machine, radius_parse): #pylint: disable=invalid-name
        ...
        self.chewie.radius_socket = Mock(**{'receive.return_value': _future('message from socket')})
        ...
        await self.chewie.receive_radius_messages()
        state_machine().event.assert_called_with(
            'fake event'
            )

    @patch("chewie.chewie.Chewie.running", Mock(side_effect=[True, False]))
    async def test_radius_output_packet_gets_packed_and_sent(self): #pylint: disable=invalid-name
        """test EAP packet creates a new state machine and is sent on"""
